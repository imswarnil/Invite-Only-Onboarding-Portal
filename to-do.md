...
- [x] **Root cause found:** `Invite_Request__c` had `enableHistory=true` at the object level, but every individual field's `trackHistory` flag was `false` — so Field History Tracking was recording _nothing_, on any field, ever. `inviteTimeline`'s empty state wasn't a bug in the widget, there was just no data to show.

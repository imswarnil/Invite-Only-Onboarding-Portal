...
You're reading this lesson *inside the React app* — same Vite build as the homepage, same
...
markdown *in the browser*, every page load. MDX instead **compiles markdown to a React component
...
`learning/` sits at the repo root, one level *above* `web/` (this Vite project's actual root).
...
- **Pagination** = the *same* sorted flat list; "next" is `sorted[currentIndex + 1]`, "prev" is
